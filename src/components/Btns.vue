...
<template>
  <v-container fluid>
    ...
    >
      ...
      <v-tab v-for="(tab,i) in tabs" :key="i" :href="tab.disabled ? tab.href : '#tab-' + i" :target="tab.target">
        {{ tab.name }}
        <v-icon v-if="icons">
          {{ tab.icon }}
          <!-- mdi-phone -->
      ...
      <!-- <v-tab-item v-for="(tab,i) in tabs" :key="i" :value="tab.disabled ? '' : 'tab-' + i" :text="tab.text">
    ...
    <v-tabs-items v-model="model">
      ...
      <v-tab-item v-for="(tab,i) in tabs" :key="i" :value="tab.disabled ? '' : 'tab-' + i">
        <v-card flat tile>
          <v-card-text>
            <h2>{{ tab.name }}</h2>
            {{ tab.text }}
